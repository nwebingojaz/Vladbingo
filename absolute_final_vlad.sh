#!/bin/bash
# VLAD BINGO - ABSOLUTE FINAL BUSINESS ENGINE (SYNCED & STABLE)
...
def get_game_info(request, game_id, tg_id):
    try:
        ...
        # Get the first card the user joined with in this specific game
        user_data = game.players.get(str(tg_id), [1])
        card_num = user_data[0] if isinstance(user_data, list) else user_data
        card = PermanentCard.objects.get(card_number=card_num)
        return JsonResponse({
            'card_number': card.card_number, 'board': card.board,
            'prize': round(prize, 2), 'status': game.status,
            'called_numbers': game.called_numbers, 'bet': float(game.bet_amount)
        })
...
def check_win(request, game_id, tg_id):
    try:
        user = User.objects.get(username=f"tg_{tg_id}")
        game = GameRound.objects.get(id=game_id)
        user_data = game.players.get(str(tg_id))
        card_num = user_data[0] if isinstance(user_data, list) else user_data
        card = PermanentCard.objects.get(card_number=card_num)
...
# 3. BOT MAIN (5 Rooms + Identity Gate + Multi-Hall Buttons)
