#!/bin/bash
echo "🚀 APPLYING 5x5 PREVIEW TO THE CORRECT FOLDER..."

# 1. Enter the backend folder
cd ~/vladbingo/backend

# 2. Add the Card Data View (Logic to send numbers to the UI)
# We use a clean overwrite to avoid sed errors
...
# 4. Update HTML with Preview Grid and Emerald Theme
...
<head>
    ...
    <style>
        ...
        .btn-start { background: #10b981; border-radius: 12px; padding: 15px; font-weight: 900; font-size: 1.1rem; width: 100%; }
...
<body class="antialiased select-none">
    ...
    <div class="bg-black/40 p-1 text-center border-b border-white/5">
        <span class="text-[9px] text-emerald-400 font-bold uppercase">Lobby Closes: </span>
...
cd ~/vladbingo
git add .
git commit -m "UI: Final 5x5 Preview and correct folder targeting"
git push -f origin main
echo "✅ PREVIEW SYSTEM UPDATED ON THE CLOUD!"
