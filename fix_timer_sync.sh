...
<body class="antialiased select-none">
    ...
    <script>
        ...
        function refreshLobby() {
            fetch('/api/lobby-info/'+uid+'/').then(r=>r.json()).then(d => {
                ...
                d.rooms.forEach(room => {
                    ...
                    // THE FIX: Only update the top timer for the room we clicked on!
                    if(activeBet == room.bet) {
                        updateBanner(room.status, room.called_count);
                        if(isLive) {
                            document.getElementById('timer').innerText = "PLAYING...";
                        } else {
                            let m = Math.floor(room.time_left / 60);
                            let s = room.time_left % 60;
        ...
        function selectRoom(t) { activeBet=t; document.getElementById('nav-room').innerText=t; document.getElementById('v-lobby').classList.add('hidden'); document.getElementById('v-selector').classList.remove('hidden'); initGrid(); refreshLobby(); }
        ...
        // Polling set to 1 second for a smooth ticking clock!
        refreshLobby(); setInterval(refreshLobby, 1000);
    </script>
</body>
</html>
INNER
