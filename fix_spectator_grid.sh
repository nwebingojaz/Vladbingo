#!/bin/bash
echo "🎲 UPGRADING 1-75 TRACKER GRIDS TO B-I-N-G-O ROW ORDER..."

cd ~/vladbingo/backend

# 1. UPDATE VIEWS (Send called_numbers to spectators)
...
<body class="antialiased select-none">
    ...
    <script>
        ...
        function buildTracker(containerId) {
            ...
            if (container.children.length > 0) return; // Only build once
        ...
        function updatePlayHall() {
            ...
            fetch(`/api/game-info/${myActiveGameId}/${uid}/`).then(r=>r.json()).then(d => {
                ...
                d.board.forEach(row => row.forEach(val => {
                    let cell = document.createElement('div'); cell.className = 'w-12 h-12 flex items-center justify-center text-lg font-black rounded-lg border border-blue-900/50 bg-slate-900 shadow-inner';
        ...
        function hitBingo() { fetch(`/api/check-win/${myActiveGameId}/${uid}/`).then(r=>r.json()).then(d => { if (d.status === 'WINNER') { tg.showAlert(`🎉 BINGO! YOU WON ${d.prize} ETB!`); location.reload(); } else if (d.status === 'NOT_YET') { tg.showScanQrPopup({text: "Lines not completed yet!"}); setTimeout(() => tg.closeScanQrPopup(), 1500); } else { tg.showAlert("Waiting for numbers."); } }); }
...
cd ~/vladbingo
git add .
git commit -m "UI: Added Authentic BINGO Grid for Spectators"
git push -f origin main
echo "✅ SPECTATOR GRID DEPLOYED!"
