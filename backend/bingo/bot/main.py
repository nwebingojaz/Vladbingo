...
def submit_deposit(request):
    if request.method == "POST":
        try:
            ...
            # FIX: Capture the transaction object so we can get its ID!
            ...
            # FIX: Included the exact ID and a fast approve command!
...
def submit_withdrawal(request):
    if request.method == "POST":
        try:
            ...
            # FIX: Capture the transaction object!
            tx = Transaction.objects.create(
                agent=user, 
                amount=amount, 
