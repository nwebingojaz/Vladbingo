#!/bin/bash
# VLAD BINGO - ULTIMATE INTEGRATED ENGINE (5 ROOMS + 20% CUT + REFUNDS)
...
# 2. FULL MODELS (Persistent Balance & States)
...
# 3. VIEWS (Home Fix + 20% Cut + Tiered Win Logic)
...
# 4. BOT MAIN (5 Rooms + Refund Logic + Auto-Timer)
...
async def start(update: Update, context):
    ...
    kbd.append([InlineKeyboardButton("💵 Join 10", callback_data="r_10"), InlineKeyboardButton("💵 Join 20", callback_data="r_20")])
...
async def btn_handler(update, context):
    ...
    if q.data.startswith("r_"):
        ...
        # Refund logic: Return money for all cards in LOBBY status
...
# 6. SAFE BUILD SCRIPT (No DROP SCHEMA)
