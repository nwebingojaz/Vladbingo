...
def join_room(request, tg_id, bet, card_num):
    ...
    if user.operational_credit < bet:
        return JsonResponse({'status': 'error', 'error': 'Low Balance'})
    game, _ = GameRound.objects.get_or_create(status="LOBBY", bet_amount=bet)
    if card_num in game.players.values():
        return JsonResponse({'status': 'error', 'error': 'Card Taken'})
...
def check_win(request, game_id, tg_id):
    try:
        ...
        # Rule: 10-40 needs 2 lines | 50-100 needs 3 lines
...
# 4. BOT MAIN (Simple 1-Button Entrance)
