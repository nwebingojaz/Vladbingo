#!/bin/bash
echo "📢 ADDING PERFECT BINGO BUTTON LOGIC..."

cd ~/vladbingo/backend

# 1. UPDATE VIEWS (Strict Win Checker)
...
# 2. UPDATE HTML (Pop-ups and Celebration)
...
<body class="antialiased select-none">
    ...
    <script>
        ...
        function hitBingo() {
            ...
            if(marked.length === 0) return tg.showPopup({title: "⚠️ Hold on!", message: "You haven't marked any numbers yet!"});
            ...
            fetch(`/api/check-win/${myActiveGameId}/${uid}/?marked=${marked.join(',')}`).then(r=>r.json()).then(d => {
                if (d.status === 'WINNER') {
                    ...
                    document.querySelectorAll('#play-card > div').forEach(cell => {
                        let clone = cell.cloneNode(true); clone.className = 'win-grid-cell';
                ...
                else if (d.status === 'NOT_YET') { tg.showPopup({title: "❌ Not Bingo Yet!", message: "You missed a number or don't have enough lines for this room! Keep watching."}); } 
                else { tg.showPopup({title: "⏳ Waiting...", message: "Waiting for the dealer to draw more numbers."}); }
            });
        }
...
git add .
git commit -m "Gameplay: Perfected the BINGO button with native Pop-ups and Celebration"
git push -f origin main
echo "✅ BINGO BUTTON LOGIC DEPLOYED! Watch Render update."
